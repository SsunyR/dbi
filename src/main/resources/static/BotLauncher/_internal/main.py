...
def run():
    ...
    async def on_command_error(ctx, error):
        if isinstance(error, commands.errors.CommandNotFound):
            attempted_cmd = ctx.message.content.split()[0][1:]
            await ctx.send(f"❌ `{attempted_cmd}` 명령어를 찾을 수 없습니다.\n"
                         f"사용 가능한 명령어 목록을 보려면 `!help`를 입력하세요.")
            print(f"존재하지 않는 명령어: {attempted_cmd}")
        else:
